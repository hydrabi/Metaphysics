...
- (NSString *)description {
    ...
    if([self.lunarYear length]){
        [desc appendFormat:@", %@", self.lunarYear];
    }
...
- (NSString*)lunarYear {
    if(_lunarDate) {
        const char *lunarYear = _calendar->lunarYear(_calendar);
        if(lunarYear) {
            _lunarYear = [[NSString alloc] initWithUTF8String:lunarYear];
        }
    }
    return _lunarYear;
}
